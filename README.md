...
| :---: | :---: | :---: | :---: |
| **۱** | `MessageSender.java` | ویرایش (افزودن متد) | افزودن متد **`SendSms`** به واسط.این کار واسط را گسترده تر می کند و یک نقض عمدی `ISP` را برای پیاده سازی ها تحمیل می کند.
| **۲** | `EmailSender.java` | ویرایش (افزودن متد) | پیاده سازی اجباری متد نامرتبط **`sendSms`** برای انطباق با واسط گسترش یافته.|
| **۳** | `SmsSender.java` | افزودن (کلاس جدید) | ایجاد کلاس جدید **`SmsSender`** و پیاده سازی `MessageSender`. این کلاس نیز مجبور به پیاده سازی متد نامرتبط `sendEmail`شده است. |
| **۴** | `ReservationService.java` | ویرایش (افزودن بلوک شرطی) | افزودن یک بلوک **`case SMS`** درون `switch (this.notifier)`  |
...
| :---: | :---: | :---: | :---: |
| **۵** | `PaymentMethods.java` | `ONSITE` (افزوده شده به `enum`) | افزودن نوع پرداخت جدید به سیستم |
| **۶** | `PaymentProcessor.java` | `public void payOnSite(double amount)` | افزودن متد اجرای منطق پرداخت حضوری |
| **۷** | `ReservationService.java` | افزودن `case ONSITE:` و فراخوانی `payOnSite()` | افزودن شرط برای هندل کردن پرداخت حضوری در ساختار `switch` |
...
##  گام ۴: بررسی مجدد تغییرات مورد نیاز (پس از اعمال SOLID)

فرض کردیم که گام ۱ را برای کد اصلاح شده (پس از انجام گام‌های ۲ و ۳) اجرا کرده‌ایم.

### ۱. در این صورت از انجام کدام یک از تغییرات ثبت شده در جدول گام ۱ معاف خواهیم شد؟

در این صورت از تمام مراحل، ۱ و ۲ و ۳ و ۴ و ۵ و ۶ و ۷ معاف می‌شویم.  

### ۲. تعداد تغییرات مورد نیاز، چند تغییر خواهد شد؟

تعداد تغییرات مورد نیاز، ۴ تا خواهد بود:
...
به کارگیری اصول **SOLID** ، سیستم رزرو را به یک مجموعه از کامپوننت‌های مستقل و قابل جایگزینی تبدیل کرده است. این امر باعث می‌شود که برنامه نه‌تنها قابلیت نگهداری بالایی داشته باشد، بلکه برای هرگونه گسترش آتی (چه افزودن یک روش پرداخت جدید، چه یک قانون تخفیف پیچیده، یا یک روش اعلان جدید) آماده باشد، بدون اینکه نیاز به تغیرات گسترده در کد یا ایجاد اختلال در کدهای اصلی و تثبیت شده باشد.
