...
| **CRP** | برقراری | `ReservationService.java`<br>`PaymentProcessor.java`<br>`Reservation.java` | کلاس `ReservationService` برای استفاده از قابلیت‌های پرداخت، از **ترکیب** (`Composition`) با `PaymentProcessor` استفاده کرده است.<br>کلاس **`Reservation`** برای استفاده از داده‌های `Room` و `Customer` به جای ارث‌بری از آن‌ها، از ترکیب استفاده کرده است و این کلاس نمونه‌ای از `Room` و `Customer` را به عنوان فیلد نگهداری می‌کند. |
