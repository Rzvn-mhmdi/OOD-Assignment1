...
| **OCP** | نقض | `ReservationService.java` | کلاس برای تغییر بسته نیست. با اضافه شدن هر روش جدید پرداخت (مثلاً ONSITE) یا اعلان (مثلاً SMS)، بلوک‌های `switch` باید تغییر کنند. <br><br> علاوه بر این، برای اضافه کردن تخفیف شهر جدید (مثلاً رم)، مجبور هستیم یک بلوک `else if` جدید به کد موجود اضافه کنیم. |
...
| :---: | :---: | :---: |
| **SRP** | کلاس `PaymentProcessor.java` چندین وظیفه متفاوت (پرداخت با کارت، نقدی، پی‌پال) را در یک کلاس واحد انجام می‌داد. در نتیجه، این کلاس چندین دلیل برای تغییر داشت. | کلاس `PaymentProcessor` حذف شد و به واسط `PaymentService` و کلاس‌های تفکیک‌شده (مانند `CardPaymentService`، `PayPalPaymentService` و `CashPaymentService`) تبدیل شد. |
