...
| :---: | :---: | :---: |
| **(SRP)** | کلاس `PaymentProcessor.java` چندین وظیفه متفاوت (پرداخت با کارت، نقدی، پی‌پال) را در یک کلاس واحد انجام می‌داد. در نتیجه، این کلاس چندین دلیل برای تغییر داشت. <br><br> علاوه بر این، کلاس `ReservationService` نیز چندین مسئولیت مستقل را با هم ترکیب کرده بود: **مدیریت رزرو و اعمال تخفیف، مدیریت پرداخت، و مدیریت اطلاع‌رسانی** (سه دلیل برای تغییر). <br><br> **وابستگی مستقیم:** **`ReservationService`** مستقیماً به جزئیات پیاده‌سازی متدهای اختصاصی (مانند `payByCard`) در `PaymentProcessor` وابسته بود. | **کلاس `PaymentProcessor` حذف شد** و به واسط **`PaymentService`** و کلاس‌های پیاده‌ساز تفکیک‌شده (مانند `CardPaymentService`، `PayPalPaymentService` و ...) تبدیل شد. <br><br> **برای `ReservationService`:** وظایف اضافی (تخفیف، چاپ فاکتور) به کلاس‌های تخصصی منتقل شدند.<br><br> **رفع وابستگی مستقیم:** وابستگی به `PaymentProcessor` حذف شد. متغیر `processor` از نوع **واسط** تعریف شد و اجرای نهایی عملیات پرداخت تنها با یک فراخوانی عمومی (`processor.processPayment(...)`) انجام گرفت. <br><br> **SRP نهایی:** وابستگی به سرویس‌های تخصصی (پرداخت، اطلاع‌رسانی، تخفیف، چاپ) از طریق **تحویل دادن شیء (Object Passing) به متد** انجام شد. این امر `ReservationService` را به یک **هماهنگ‌کننده** (Orchestrator) صرف تبدیل کرد و **SRP به طور کامل برقرار شد**. |
| **OCP** | **مشکل در کد اولیه:** کلاس **`ReservationService`** به شدت به جزئیات پیاده‌سازی متدهای پرداخت و اطلاع‌رسانی وابسته بود. این وابستگی از طریق استفاده از بلوک‌های **`switch`** برای مدیریت Enumsهای `PaymentMethods` و `Notifier` ایجاد شده بود. در نتیجه، افزودن هر روش پرداخت یا اعلان‌کننده جدید نیازمند **تغییر** کد داخلی متد `makeReservation` بود. این نقض مستقیم اصل OCP است. | **راه حل در کد تغییر یافته:** برای برقراری اصل OCP، از **الگوی استراتژی (Strategy Pattern)** و **تزریق وابستگی (Dependency Injection)** استفاده شد. متد `makeReservation` اکنون **اینترفیس‌ها** (مانند **`PaymentService`** و **`MessageSender`**) را به عنوان پارامتر دریافت می‌کند. با این کار، کلاس `ReservationService` دیگر نیازی به دانستن نوع پرداخت یا اعلان‌کننده ندارد و فقط متدهای عمومی تزریق شده را فراخوانی می‌کند. این ساختار تضمین می‌کند که کلاس برای **تغییر بسته** و برای **گسترش باز** است. |
