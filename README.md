...
| **SRP** | `ReservationService.java`<br>`PaymentProcessor.java` | **`ReservationService`** بیش از یک مسئولیت (رزرو و مدیریت تخفیف، پرداخت و اطلاع‌رسانی) را بر عهده دارد.<br>کلاس **`PaymentProcessor`** مسئولیت انجام چند نوع عملیات پرداخت مجزا (کارت، نقد، پی‌پال) را بر عهده دارد. |
